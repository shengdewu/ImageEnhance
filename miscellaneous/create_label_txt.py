...
def _create_dup_dict(path, name_key=None):
    name_dict = dict()
    for name in _search_rt(os.path.join(path)):
...
def create_train_no_aug_label(root_path, max_rts=-1):
    gts = _search_rt(os.path.join(root_path, 'gt_8bit_1500p.txt'))
    rt_input_dict_list = list()

    if max_rts != 0:
        rts = _search_rt(os.path.join(root_path, 'rt_8bit_1500p.txt'))
        if max_rts > 1:
    ...
    # rt_input_dict_list.append(_create_dup_dict(os.path.join(root_path, 'rt_tif_16bit_540p_overexposure.only.light.v2_01.txt')))
    # rt_input_dict_list.append(_create_dup_dict(os.path.join(root_path, 'rt_tif_16bit_540p_underexposure.only.light.v2_01.txt')))
    # rt_input_dict_list.append(_create_dup_dict(os.path.join(root_path, 'rt_tif_16bit_540p_overexposure.only.light.light.light_01.txt')))
    # rt_input_dict_list.append(_create_dup_dict(os.path.join(root_path, 'rt_tif_16bit_540p_underexposure.only.light.dark.dark_01.txt')))
    # rt_input_dict_list.append(_create_dup_dict(os.path.join(root_path, 'rt_tif_16bit_540p_underexposure.only.light.light.light_01.txt')))
    # rt_input_dict_list.append(_create_dup_dict(os.path.join(root_path, 'rt_tif_16bit_540p.only.light.local.light.v2_01.txt')))
    ...
    _label(root_path, train_pair, 'all_jpg_test')


if __name__ == '__main__':
    create_train_no_aug_label('/mnt/sdb/data.set/xintu.data/enhance.data/xt.image.enhancement.540.jpg', 1000)